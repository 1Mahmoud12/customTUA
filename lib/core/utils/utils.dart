...
class Utils {
  ...
  static TextSpan highlightText({required String text, bool isExpanded = false}) {
    ...
    if (!isExpanded) {
      ...
      for (final String word in words) {
        if (word.startsWith('#')) {
          spans.add(TextSpan(text: '$word ', style: const TextStyle(color: AppColors.secondPrimaryColor, fontWeight: FontWeight.bold)));
        } else {
  ...
  static const platform = MethodChannel('com.example.dialer/dialer');

  static Future<void> launchDialer(String number) async {
    try {
      await platform.invokeMethod('dialPhoneNumber', {'number': number});
    } on PlatformException catch (e) {
      log('Failed to open dialer: ${e.message}');
    }
  }
  ...
  // static String? buildCountryCodeId({required int countryCodeId}) {
  //   for (var i = 0; i < ConstantsModels.countryCodeModel!.data!.length; i++) {
  //     if (ConstantsModels.countryCodeModel!.data![i].id == countryCodeId) {
  //       return ConstantsModels.countryCodeModel!.data![i].code;
  //     }
  //   }
  //   return null;
  // }
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  Text(title, style: Styles.style18300.copyWith(fontWeight: FontWeight.w400, color: AppColors.textColor)),
                  ...List.generate(
                    subTitle.length,
                    (index) => Column(
                      children: [
                    ...
                    children: [
                      Expanded(
                        child: CustomTextButton(
                          ...
                          child: Text(nameConfirmButton, style: Styles.style16400.copyWith(fontWeight: FontWeight.w500, color: AppColors.textColor)),
                          onPress: () {
                      ...
                      Expanded(
                        ...
                        child: CustomTextButton(
                          ...
                          child: Text(nameCancelButton, style: Styles.style16400.copyWith(fontWeight: FontWeight.w500, color: AppColors.white)),
                          onPress: () {
...
Future<void> languagesModalBottomSheet({required BuildContext context}) {
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  Text('languages'.tr(), style: Styles.style18300.copyWith(fontWeight: FontWeight.w400, color: AppColors.textColor)),
                  const SizedBox(height: 24),
                  Container(
                    decoration: ShapeDecoration(
                      color: Colors.white,
                  ...
                  CustomTextButton(
                    backgroundColor: AppColors.white,
                    borderColor: AppColors.textColor,
                    child: Text('Save'.tr(), style: Styles.style16400.copyWith(fontWeight: FontWeight.w500, color: AppColors.textColor)),
                    onPress: () {
...
Future<DateTime> customShowDatePicker({required BuildContext context}) {
  return showDatePicker(
    builder:
        (context, child) => Theme(
          data: Theme.of(context).copyWith(colorScheme: const ColorScheme.light(primary: AppColors.primaryColor, onSecondary: AppColors.white)),
          child: child!,
        ),
    context: context,
