...
class UtilsGoogleMap {
  ...
  static Future<Marker> addMarker({
    ...
    return Marker(markerId: MarkerId(idMarker), position: latLng, infoWindow: InfoWindow(title: title, snippet: subTitle), icon: customMarker);
  }

  static Future<void> addUpdateMyMarker({required LatLng latLng, List<Marker>? markers}) async {
    final Marker meMarker = await addMarker(idMarker: '-1', latLng: latLng, image: AppImages.addressMarker, title: 'انت هنا', width: 40.w);
  ...
  static Future<void> addUpdateCustomMarker({required LatLng latLng, List<Marker>? markers}) async {
    final Marker customMarker = await addMarker(idMarker: '-2', latLng: latLng, image: AppImages.addressMarker, width: 40.w);
